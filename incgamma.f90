...
!  real ( kind = 8 ) alngam
  ...
  cmpgamma = dexp(alngam(x, ifault))
  return
end function cmpgamma

function gammainc(a, x)
! Calculates the unnormalised complementary incomplete gamma function from the normalised incomplete gamma function
! Gamma(a, x) = integral x to infinity of exp(-t)*t^(a-1) dt = Gamma(a)*[1 - (1/Gamma(a))*integral 0 to x of exp(-t)*t^(a-1) dt]

  use amr_parameters, only: ein_n
  implicit none

  real ( kind = 8 ) a
  real ( kind = 8 ) x
  real ( kind = 8 ) gammainc
!  real ( kind = 8 ) gammad
!  real ( kind = 8 ) cmpgamma
  integer ( kind = 4 ) ifault

  gammainc = cmpgamma(a)*(1d0 - gammad(x, a, ifault))

  return
end function gammainc

function gammainc2n(x)
! Calculates gammainc(2n, x) for use with the Einasto profile, this avoids repeatedly evaluating Gamma(2n)

  use amr_parameters, only: ein_n
  implicit none

  real ( kind = 8 ) x
  real ( kind = 8 ) gammainc2n
!  real ( kind = 8 ) gammad
!  real ( kind = 8 ) cmpgamma
  real ( kind = 8 ) fac
...
function gammainc3n(x)
! Calculates gammainc(3n, x) for use with the Einasto profile, this avoids repeatedly evaluating Gamma(3n)
  use amr_parameters, only: ein_n
  implicit none

  real ( kind = 8 ) x
  real ( kind = 8 ) gammainc3n
!  real ( kind = 8 ) gammad
!  real ( kind = 8 ) cmpgamma
  real ( kind = 8 ) fac
