...
subroutine unsplit(uin,gravin,pin,flux,tmp,dx,dy,dz,dt,ngrid)
  ...
  real(dp),dimension(1:nvector,if1:if2,jf1:jf2,kf1:kf2,1:3   ,1:ndim)::tmp
#else
  ...
  ! Intermediate fluxes
#ifdef LIMIT_FLUX_OUT
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nvar+2),save::fx
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:3   ),save::tx
#else
  real(dp),dimension(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nvar+1),save::fx
...
#ifdef LIMIT_FLUX_OUT
     !Save velocity used by Riemann solver
     do l=1,ngrid
        tmp (l,i,j,k,3,1)=tx(l,i,j,k,3)
     end do
#endif
...
#ifdef LIMIT_FLUX_OUT
  !Limit outgoing mass flux to be no greater than the total mass in the cell
  do i=ilo,ihi
  do j=jlo,jhi
  do k=klo,khi
     do l=1,ngrid
        ...
        do idim=1,ndim
           ...
           if(idim==2)j0=1
           if(idim==3)k0=1
!           flux_tot = flux_tot + flux(l,i,j,k,1,idim) - flux(l,i+i0,j+j0,k+k0,1,idim)
           if (flux(l,i,j,k,1,idim) >= 0d0)then
              !inflow
              flux_in = flux_in + flux(l,i,j,k,1,idim)
           else
              !outflow
              flux_out = flux_out - flux(l,i,j,k,1,idim)
           endif
           if (flux(l,i+i0,j+j0,k+k0,1,idim) <= 0d0)then
              flux_in = flux_in - flux(l,i+i0,j+j0,k+k0,1,idim)
           else
              flux_out = flux_out + flux(l,i+i0,j+j0,k+k0,1,idim)
           endif
        enddo
        if (flux_out > flux_in + uin(l,i,j,k,1))then
           ...
           do idim=1,ndim
              ...
              if(idim==3)k0=1
              if(flux(l,i,j,k,1,idim) < 0d0)then
                 eth_flow = tmp(l,i,j,k,2,idim)
                 vel_riem = tmp(l,i,j,k,3,idim)
                 flux(l,i,j,k,1,idim) = flux(l,i,j,k,1,idim)/(1.01*flux_excess) !rescale mass density flux
                 prs = eth_flow*(gamma-1d0)/vel_riem !prs*dt/dx = (e_thermal*(gamma-1)*v*dt/dx)/v
                 do jdim=1,ndim
                    flux(l,i,j,k,1+jdim,idim) = (flux(l,i,j,k,1+jdim,idim)-prs)/(1.01*flux_excess)+prs !rescale momentum density flux
                 enddo
                 flux(l,i,j,k,ndim+2,idim) = (flux(l,i,j,k,ndim+2,idim)-(eth_flow+prs)*vel_riem)/(1.01*flux_excess) + (eth_flow+prs)*vel_riem !rescale energy density flux
#if NVAR > NDIM + 2
                 !rescale other advected quantities
                 do n = ndim+3, nvar
                    flux(l,i,j,k,n,idim) = flux(l,i,j,k,n,idim)/(1.01*flux_excess)
                 end do
#endif
              endif
              if(flux(l,i+i0,j+j0,k+k0,1,idim) > 0d0)then
                 eth_flow = tmp(l,i+i0,j+j0,k+k0,2,idim)
                 vel_riem = tmp(l,i+i0,j+j0,k+k0,3,idim)
                 flux(l,i+i0,j+j0,k+k0,1,idim) = flux(l,i+i0,j+j0,k+k0,1,idim)/(1.01*flux_excess) !rescale mass density flux
                 prs = eth_flow*(gamma-1d0)/vel_riem !prs*dt/dx = (e_thermal*(gamma-1)*v*dt/dx)/v
                 do jdim=1,ndim
                    flux(l,i+i0,j+j0,k+k0,1+jdim,idim) = (flux(l,i+i0,j+j0,k+k0,1+jdim,idim)-prs)/(1.01*flux_excess)+prs !rescale momentum density flux
                 enddo
                 flux(l,i+i0,j+j0,k+k0,ndim+2,idim) = (flux(l,i+i0,j+j0,k+k0,ndim+2,idim)-(eth_flow+prs)*vel_riem)/(1.01*flux_excess) + (eth_flow+prs)*vel_riem !rescale energy density flux
#if NVAR > NDIM + 2
                 !rescale other advected quantities
                 do n = ndim+3, nvar
                    flux(l,i+i0,j+j0,k+k0,n,idim) = flux(l,i+i0,j+j0,k+k0,n,idim)/(1.01*flux_excess)
                 end do
#endif
              endif
           enddo
           write(*,*)'WARNING: Outflow ',flux_out,' too high compared to density ', uin(l,i,j,k,1), ' and inflow ',flux_in,'! Outflow has been divided by a factor of ',1.01*flux_excess
        endif
     end do
  end do
  end do
...
#ifdef LIMIT_FLUX_OUT
  real(dp),dimension(1:nvector,ip1:ip2,jp1:jp2,kp1:kp2,1:3)::tmp
#else
...
#ifdef LIMIT_FLUX_OUT
  real(dp),dimension(1:nvector,1:nvar+2),save::fgdnv
#else
...
#ifdef LIMIT_FLUX_OUT
           ! Velocity used in Riemann flux
           do l = 1,ngrid
              tmp(l,i,j,k,2) = fgdnv(l,nvar+2)
           end do
#endif
        end do
     end do
